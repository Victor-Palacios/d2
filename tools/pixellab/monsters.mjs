...
export const MONSTERS = {
  ...
  // === Batch: the Shade tier (middle evolutions) + gap-filling rookies =====
  // 6 SHADE-tier monsters — the "next class". Composed from the shade vibe so
  // they read as lean, mildly-menacing adolescents (see the ladder rule).
  cindraw: { palette: ["#2a1408", "#f07f2a", "#ffe08a", "#ff5a2a", "#ffd23f", "#1a1a2e"], size: SIZE,
    prompt: `${STAGES.shade.vibe}, a fierce fire reptile with a flaming crest and glowing ember cracks along its scales, ${STYLE}, ${STAGES.shade.tone}` },
  galewing: { palette: ["#141a28", "#79c8e8", "#e8f6ff", "#ffffff", "#3f7fa8"], size: SIZE,
    prompt: `${STAGES.shade.vibe}, a swift winged sky-creature with long sharp feathered wings and a crested head, ${STYLE}, ${STAGES.shade.tone}` },
  thornlash: { palette: ["#12240f", "#7ec850", "#f2f7a0", "#3f8f3a", "#ffffff"], size: SIZE,
    prompt: `${STAGES.shade.vibe}, a vine-lashing plant creature with thorny tendrils and a broad leaf hood, ${STYLE}, ${STAGES.shade.tone}` },
  gearhulk: { palette: ["#101319", "#9aa7bd", "#ffd166", "#dff3ff", "#39e0ff"], size: SIZE,
    prompt: `${STAGES.shade.vibe}, a bulky machine creature with a heavy metal shell and a glowing reactor core, ${STYLE}, ${STAGES.shade.tone}` },
  tidalmaw: { palette: ["#0d2436", "#3fb6e8", "#cdf3ff"], size: SIZE,
    prompt: `${STAGES.shade.vibe}, a finned water creature with a jelly body, gills and a toothy grin, ${STYLE}, ${STAGES.shade.tone}` },
  duskmaw: { palette: ["#100a1c", "#4a3670", "#c77dff"], size: SIZE,
    prompt: `${STAGES.shade.vibe}, a shadow spirit with wispy clawed arms and glowing violet eyes, ${STYLE}, ${STAGES.shade.tone}` },

  // 4 gap-filling ROOKIES (cute) — cover the thin fire element and missing
  // class x element combos (fire/mage, fire/assassin, machine/assassin, nature/mage).
  flickit: { palette: ["#2a1408", "#ff5a2a", "#f07f2a", "#ffe08a", "#ffd23f"], size: SIZE,
    prompt: `${STAGES.wisp.vibe}, a tiny floating fire-wisp, a warm dancing flame body with a glowing core, ${STYLE}, ${STAGES.wisp.tone}` },
  cindercat: { palette: ["#2a1408", "#f07f2a", "#ffe08a", "#ff5a2a", "#1a1a2e"], size: SIZE,
    prompt: `${STAGES.wisp.vibe}, an ember-furred kitten with glowing paws and a spark-tipped tail, ${STYLE}, ${STAGES.wisp.tone}` },
  sparkfly: { palette: ["#101319", "#9aa7bd", "#39e0ff", "#dff3ff", "#ffd166"], size: SIZE,
    prompt: `${STAGES.wisp.vibe}, a mechanical dragonfly with a slim metal body and glowing buzzing wings, ${STYLE}, ${STAGES.wisp.tone}` },
  mossling: { palette: ["#12240f", "#5e7a3a", "#9fc45a", "#c9d98a", "#f2f7a0"], size: SIZE,
    prompt: `${STAGES.wisp.vibe}, a mossy forest sprite with a glowing spore cap on its head and leafy tufts, ${STYLE}, ${STAGES.wisp.tone}` },
};
