...
| `dialogueguard.mjs` | The dialogue advance-guard (`src/ui/DialogueBox.ts`, `ADVANCE_GUARD_MS`): the first line of a freshly-opened box briefly withholds its *advance*, so a burst of confirm presses leaked from the previous dialogue (or across a scene change — e.g. tapping through the tutorial + 'An echo' as the first battle's melee lesson loads) can't reveal-and-skip it in one frame. Seeds the first crossing battle, then asserts a rapid double-press does **not** skip the mechanic-tutorial line while a paced press still advances it. Regression test for the "text flashes past unread" bug. |
