...
import { openItems } from './ItemsScreen';
  ...
  for (;;) {
    const items: GridItem[] = [
      ...
      },
      {
        ...
        icon: menuIcon('gear', '#7bdc8a'),
        color: '#7bdc8a',
      },
      {
        value: 'items',
        label: 'Items',
        sublabel: 'use a keepsake',
        icon: menuIcon('items', '#ff8fa3'),
        color: '#ff8fa3',
      },
      {
        value: 'moves',
    ...
    else if (choice === 'items') await openItems(parent);
